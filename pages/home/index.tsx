...
const NotificationInFeed = ({ data }: any) => {
      ...
      <div className="md:flex flex-start">
        ...
        <div className="block p-6 rounded-lg shadow-lg bg-gray-100 max-w-md ml-6 mb-10">
          <div className="flex justify-between mb-4">
            <a href="#!" className="font-medium text-purple-600 hover:text-purple-700 focus:text-purple-800 duration-300 transition ease-in-out text-sm">{data.title}</a>
...
const NotificationList = ({ notifications }: any) => {
  ...
  return (
    <div style={{ marginLeft: '50px' }}><h3 className="text-2xl text-gray-700 font-bold mb-6 ml-3">Latest Notifications</h3><ol>
      {
        notifications.map((n: any, i: number) => <NotificationInFeed key={i} data={n} />)
      }
    </ol></div>
  )
...
const Notification = ({ marginTop, data }: any) => {

  return (
    <div className="z-50 fixed right-0 top-0 p-4" style={{ marginTop }}>
...
const NotificationBar = ({ notifications }: any) => {

  return (
    <>
      {
        notifications.map((n: any, i: number) => <Notification key={i} marginTop={`${120 * i}px`} data={n} />)
      }
    </>
  )
}
